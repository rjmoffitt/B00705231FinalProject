...
library(RODBC)
library(gridExtra)
#Libraries used in the Project. Some were just tested but not required in the Final System
...
#con = dbConnect(MySQL(), user='root', password="tv0@Oqmh", dbname='predicttrenddb', host='localhost')
...
#   
# )# was used for the past data tab


##SERVER CODE
server = function(input, output, session){
  options(scipen=999) #Removes Scientific Notation from Long Numbers
  ...
  ##Reed Job Search
    ...
    r<- GET("https://www.reed.co.uk/api/1.0/search", query = list(keywords = input$keyword), authenticate("a32b16cc-f522-494b-8a70-8513a1fd48c2", "")) #API Key
    
    parsed_content <- content(r, "parsed") #puts json content into more readable format
    parsed_content
    
    output$jobTitle1 <- renderText({
      parsed_content$results[[1]]$jobTitle
    }) #Outputs the job title of the first result
    ...
    }) # Outputs the Total Number of Jobs available
    ...
    if(input$keyword != ""){
      if (parsed_content$totalResults == 0){
        ...
        })#If no results from the search only output the total no. of jobs
        
      } else {
        output$oneJob <- renderUI({
        #UI code for displaying four containers with Job Adverts pulled from the Reed API  
  ...
  ##ScienceSubjectsGraph
    ...
    #Used for when the MySQL Database was implemented
  ...
  ##End of NonScienceSubjectsGraph
  ...
  ## End of OpenUniversityDegreeGraph
  ...
  ##End of FoundationDegreeGraph
    
  ##Degrees Page
    observe({
      if(input$country == "All" && input$mode == "Full-Time"){
        subAreaTS = ts(ukSubjectFT[(as.numeric(input$subArea))], start=2000) 
        #Sets a Time Series Object for the data frame selected in the subArea Dropdown Menu starting from the Year 2000
        subAreaTSTrain = window(subAreaTS, start = 2000, end = 2014) # Training set
        subAreaTSTest = window(subAreaTS, start = 2015) # Test Set
        
        trainArima = auto.arima(subAreaTSTrain, stepwise = T, approximation = F, trace = T) # Auto.Arima calculated for the Training Set
        testArima = auto.arima(subAreaTSTest, stepwise = T, approximation = F, trace = T) # Auto.Arima calculated for the Test set
        trainETS = ets(subAreaTSTrain) # Exponential Smoothing model calculated on the training set
        testETS = ets(subAreaTSTest) # Exponential Smoothing model calculated on the test set
        trainHolt = holt(subAreaTSTrain, h = 4) # Holt Winters model calculated on the train set with a forecast of four years set.
        testHolt = holt(subAreaTSTest, h = 1) # Holt Winters model calculated on the test set with a forecast of one year set.
        trainHoltDamped = holt(subAreaTSTrain, h = 4, damped = T) # Holt Winters damped model calculated on the test set with a forecast of four years set.
        testHoltDamped = holt(subAreaTSTest, h = 1, damped = T) # Holt Winters damped model calculated on the test set with a forecast of one year set.
        ...
        #Auto.arima model, exponential smoothing model, holt winters and holt winters damped models used on the full data frames.
        ...
        #Displays the AICc Akaike Information Criterion for each of the four models. Estimator of out of sample prediction error
        ...
        #RMSE values for each model stored within variables
        #Root Mean Squared Error is stored within the second subscript of the accuracy() function
        #rmse <- function(error) {sqrt(mean(error^2))} # The formula to work out the Root Mean Squared Error
        ...
        #MAPE values for each model stored within variables
        #Mean Absolute Percentage Error Loss is stored within the fifth subscript of the accuracy() function
        ...
        } #IF statements used to detect the model with the lowest RMSE to display in the Graph
        
        output$subModel <- renderValueBox(
          valueBox("Forecast Model:", modelChoice)
        )# This was used for testing so that on the viewer the developer could see which model was used in the Graph
        ...
        )# This was used for Testing so that the developer could see each of the RMSE, MAPE and AICC values for each model.
        
        roundedSub <- round(plotSubArea$mean, 0) # rounds the forecasted values
        
        predictedSubValues <- as.data.frame(c(subAreaTS, roundedSub)) 
        # Joins the Time series element with the forecasted values to create a longer data frame to display in the table below the graph
        subYear <- c(2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027, 2028, 2029, 2030, 2031, 2032, 2033) 
        #Year figures to add to the bottom of the table alongside forecasted figures
        predicSub <- append(ukSubjectFT$year, subYear[1:input$subYearsPredicted]) 
        # Joins the past Academic Years with the number of forecasted years selected in the numbers predicted slider to be displayed in the table.
        subAreaTable <- cbind(predicSub, predictedSubValues) # This creates the table with two columns. One with past and forecasted Years and one with past and forecasted student numbers.
        names(subAreaTable)[1] <-"Academic Year Beginning" #Renames the first column heading
        names(subAreaTable)[2] <-"No. Of Students" # Renames the second column heading
        pSubValues <- c(subAreaTS, roundedSub) #Combines the time series object with the rounded forecasted values
        
        differenceV <- pSubValues[19+input$subYearsPredicted]-pSubValues[19] #difference between the last forecasted value and the last recorded past data.
        percentageV <- round((differenceV/pSubValues[19])*100, 2) #Calculates the percentage increase or decrease 
        
        # output$subTable <- renderTable(
        #   
        #   subAreaTable, striped = TRUE, bordered = TRUE, hover = TRUE, responsive = TRUE
        # ) Old Table format
        output$subTable <- renderDataTable(datatable(subAreaTable, list(paging=F), rownames = F)) # Table that displays the past and future data
        
        output$subGraph <- renderPlot({
          autoplot(plotSubArea)+
            labs(caption = "Source: Higher Education Statistics Agency")+
            xlab("Academic Year Beginning")+ ylab("No. of Students") + theme(text = element_text(size=15)) # Text Size 15
        }) #The graph that displays the past and future data. 
        ...
        } # IF Statements used to work out if the last forecasted value is greater or less than the last predicted data. 
        # This decided which renderText item to display
        ...
        output$subTable <- renderDataTable(datatable(subAreaTable, list(paging=F), rownames = F))
  ...
  ##OpenUni Degree Forecast
    ...
    output$openUniTable <- renderDataTable(datatable(openTable,
                                                         list(paging = F),
                                                         rownames = F)
  ...
  ##Foundation Degree Page
    ...
    output$foundationTable <- renderDataTable(datatable(foundTable, list(paging=F), rownames = F))
  ...
  ##Gender Information Tab
    ...
    output$genderModel <- renderValueBox(
      valueBox("Forecast Model:", modelChoice)
    ) #used for test purposes only no longer outputted in UI
    ...
    percentageGenV <- round((differenceGenV/pGenValues[19])*100, 2)
     ...
     output$genderTable <- renderDataTable(datatable(genTable, list(paging=F), rownames = F))
  ...
  ##Age Information Tab
    ...
    output$ageModelTable <- renderDataTable(datatable(ageTable, list(paging=F), rownames = F))
  ...
  ##Ethnicity Information Tab
    ...
    output$ethnicTable <- renderDataTable(datatable(ethTable, list(paging=F), rownames = F))
  ...
  #Disability Information Tab
    ...
    output$disabilityTable <- renderDataTable(datatable(disTable, list(paging=F), rownames = F))
  ...
  ##Job Vacancies Tab
    ...
    names(jobTable)[2] <- "No. Of Jobs Available (Thousands)"
    ...
    output$jobVacTable <- renderDataTable(datatable(jobTable, list(paging=F), rownames = F))
    
    output$jobPlot <- renderPlot ({
      autoplot(plotJob) +
        labs(title = "Number of Jobs Available per Job Category relevant to University", 
    ...
    } else if (pJobValues[19+input$jobYearsPredicted] < pJobValues[19]){
      output$jobPercent <- renderText(
  ...
  # }) #Code designed to add buttons on the Home Page that navigated to different TabPanels (Pages)
  
  
  # session$onSessionEnded (function(){
  #   odbcClose(con)
  # }) # code implemented to close the DB connection once the session ended
}

##UI CODE
ui = navbarPage(theme = shinytheme("cerulean"), title = ("Predicting Education Trends"), footer = includeHTML("footer.html"), id="navbarId",
    #Footer designed in HTML via Visual Studio Code            
                tabPanel("Home", id="tabHome", 
                         fluidRow( # Keeps elements within on the same line away from the next fluid row element
                           div(
                             tags$img(src="banner2.jpg", class="img-responsive"), # img responsive class enables the Banner to conform to all sizes of Webpage windows / devices
                             div(
                         ...
                         #HOW
                         fluidRow(
                           column(2),# column with a width of 2. Allows for a gap to the left hand side of the code. A fluid row width is maximum 12.
                           column(8, # The main code within this fluid row will spread across a width of 8 out of 12.
                                  div(
                           ...
                           column(2) # Column width of two to leave another gap on the right hand side of the main code. This keeps graphs and images from getting too stretched when open the browser on full screen
                         ),
                         ...
                         Will there be any change in the Subject Areas with the most Jobs?
                         ...
                         conditionalPanel(condition="parse_content.totalResults>0",
                                          uiOutput("oneJob")
                         ), # IF there are results from the reed search then carry out the code in this UI Output
                         
                         uiOutput("numberResults"), #always output number of results for each search
    ...
    #Start of Student Characteristics Page
                tabPanel("Student Characteristics", id= "tabStud",
                         navbarPage(theme = shinytheme("cerulean"), title = ("Student Characteristics"), id="navbarstud",
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,
                                                          ...
                                                          #NUmber of Years Predicted Slider
                                                          
                                                          
                                                          plotOutput("genderPlot"), # Graph 
                                                          ...
                                                          dataTableOutput("genderTable")
                                                          
                                                          #valueBoxOutput("genderModel"), # For Testing Purposes - Displayed the chosen Forecast Model
                                                          #verbatimTextOutput("genderTextOut") # For Testing Purposes - Displayed the RMSE values for each Model
                                                   ),
                                                   column(2)
                                                 )
                                             )
                                    ), # End of Gender Information Page
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,   
                                                          ...
                                                          tags$br(), # Line Break
                                                          dataTableOutput("ageModelTable")
                                                          #valueBoxOutput("ageModel"), #Testing Purposes
                                                          #verbatimTextOutput("ageTextOut") # Testing Purposes
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,
                                                          ...
                                                          tags$br(),
                                                          dataTableOutput("disabilityTable")
                                    ...
                                    ) #End of Disability Page
                                    
                         ) # End of NavBarPage
                ), #End of Student Characteristics Page
                
    #Start of Subject Data Page
                tabPanel("Subject Data", id = "tabSub",
                         navbarPage(theme = shinytheme("cerulean"), title = ("Subject Data"), id="navbarsub",
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,
                                                          ...
                                                          plotOutput("scienceSubAreaGraph"), #Graph
                                                          tags$hr(),
                                                          h2("Non-Science Degree Subjects"),
                                                          plotOutput("nonScienceSubAreaGraph"), #Graph
                                                          tags$hr(),
                                                          h1("Foundation Degree Subjects"),
                                                          plotOutput("foundationGraph"), #Graph
                                                          tags$hr(),
                                                          h1("Open University Subjects"),
                                                          plotOutput("openUniGraph") #Graph
                                                   ),
                                                   column(2)
                                                 )
                                             )
                                    ), # End of Subject Data Overview Page
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,
                                                          ...
                                                          dataTableOutput("openUniTable")
                                                   ),
                                                   column(2)
                                                 )
                                             )
                                    ), # End of Open University Degrees Page
                                             ...
                                             div(align="center",
                                                 fluidRow(
                                                   ...
                                                   column(8,
                                                          ...
                                                          dataTableOutput("foundationTable")
                                                   ),
                                                   column(2)
                                                 )
                                             )
                                    ) # End of Foundation Degrees Page
                ...
                # 
                #  ) # Removed Past Data Page
                
                
                
)
shinyApp(ui = ui, server=server) # End of Shiny App Code
